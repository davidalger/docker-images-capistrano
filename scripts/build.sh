...
VERSION_LIST="${VERSION_LIST:-"3.14"}"

for CAPISTRANO_VERSION in ${VERSION_LIST}; do
  IMAGE_TAG="davidalger/capistrano:${CAPISTRANO_VERSION}"
  export CAPISTRANO_VERSION

  printf "\e[01;31m==> building ${IMAGE_TAG}\033[0m\n"
  docker build -t "${IMAGE_TAG}" --build-arg CAPISTRANO_VERSION ${BASE_DIR}
